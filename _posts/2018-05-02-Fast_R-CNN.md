...
- "brute learning" learning: 在训练与测试阶段, 均采用固定的 scale 进行 resize

- image pyramids： 在训练阶段, 对已被加入 minibatch 的图片, 在 image pyramids 中为其随机选取一个 scale 进行 resize

> 注： 项目源码里的　resize 是这样的, 最短边不大于固定 scale, 长边不超过 max_size, 并保持比例
